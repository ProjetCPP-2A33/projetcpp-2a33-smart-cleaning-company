...
void MainWindow::processEmployeID(const QString &employeID)
{
    ...
    if (!employeeCardCode.isEmpty()) {
        ...
        i=1;
        arduino.write_to_arduino(employeeFullNameData,i);
       // Envoyer au Arduino
    } else {
        ...
        i=2;
        arduino.write_to_arduino(errorMessage,i);
           // Envoyer le message d'erreur à Arduino
    }
}
...
void MainWindow::setupConnections()
{
    ...
    //connect(ui->pushButtonverifer, &QPushButton::clicked, this, &MainWindow::verifId);
    //f
...
/*void MainWindow::verifId() {
    QString id = ui->lineEditEmployeeID->text();  // Get ID from QLineEdit
...
//MAHDIIIIIIIIIIIIIIIIIIIIIIIIIIIiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii
...
bool MainWindow::checkIfRFIDExists(const QString &rfidID) {
    ...
    // Requête pour vérifier si le tag RFID existe
    query.prepare("SELECT COUNT(*) FROM EMPLOYE WHERE CARDUID = :CRUID");
    query.bindValue(":CRUID", rfidID.toLower()); // Normaliser le tag

    if (query.exec()) {
        if (query.next() && query.value(0).toInt() > 0) {
            return true; // RFID existe dans la base de données
        }
    } else {
        qDebug() << "Échec de la requête sur la base de données :" << query.lastError().text();
        ui->there->setText("Erreur : Échec de la requête sur la base de données.");
    }

    return false; // RFID n'existe pas
}

void MainWindow::processRFID(const QString &rfidID) {
    QString employeeCardCode = rfidID;
    QString result_poste;
    int i = -1;

    if (checkIfRFIDExists(employeeCardCode)) {
        // Si l'ID existe, récupérer le poste et l'ID de l'employé
        QString employeID = getemployeIDFromDatabase(employeeCardCode).toUpper();
        QString employeposte = getemployeposteFromDatabase(employeeCardCode).toUpper();

        result_poste = "POSTE: " + employeposte + ", ID: " + employeID;

        ui->labelResult->setText("Bienvenue, " + result_poste); // Afficher sur le QLabel
        QByteArray employeeFullNameData = result_poste.toUtf8();
        i = 1; // Code pour indiquer un accès accordé
        arduino.write_to_arduino(employeeFullNameData, i); // Envoyer les données à Arduino
    } else {
        result_poste = "ERREUR : Carte non trouvée"; // Message d'erreur
        ui->labelResult->setText("Accès refusé : " + result_poste); // Afficher sur le QLabel
        QByteArray errorMessage = result_poste.toUtf8();
        i = 2; // Code pour indiquer un accès refusé
        arduino.write_to_arduino(errorMessage, i); // Envoyer les données à Arduino
    }
}

void MainWindow::readSerialData() {
    static QByteArray buffer; // Buffer pour stocker les données série incomplètes
    buffer.append(serial->readAll());

    // Vérifier si un tag RFID complet est reçu (par exemple, se termine par une nouvelle ligne)
    if (buffer.contains('\n')) {
        QString rawData = QString(buffer).trimmed();
        buffer.clear(); // Vider le buffer après traitement

        qDebug() << "Données brutes reçues :" << rawData;

        // Extraction de l'UID du tag RFID (e.g., "UID tag : 13 B1 6A A9")
        if (rawData.startsWith("UID tag :")) {
            QString rfidID = rawData.mid(10).remove(" ").toLower(); // Normaliser l'UID
            qDebug() << "ID RFID formaté :" << rfidID;

            processRFID(rfidID); // Traiter l'ID RFID
        } else {
            qDebug() << "Format des données série inattendu :" << rawData;
            ui->there->setText("Erreur : Données invalides reçues.");
        }
    }
}
