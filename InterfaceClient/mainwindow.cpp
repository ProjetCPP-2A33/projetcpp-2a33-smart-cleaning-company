...
/*void MainWindow::exporterPDF()
{
    ...
    QString fileName = QFileDialog::getSaveFileName(this, tr("Enregistrer en PDF"), "", tr("PDF Files (*.pdf)"));
    if (fileName.isEmpty()) // Si aucun fichier n'est choisi, annuler l'opération
        return;

    // Forcer l'extension PDF si elle est absente
    if (!fileName.endsWith(".pdf", Qt::CaseInsensitive))
        fileName += ".pdf";

    // Configurer le fichier PDF
    QPrinter printer(QPrinter::PrinterResolution);
    printer.setOutputFormat(QPrinter::PdfFormat);
    printer.setOutputFileName(fileName);

    // Initialiser QPainter pour dessiner dans le fichier PDF
    QPainter painter;
    ...
    // Dessiner le titre
    int yPosition = 30; // Position verticale de départ
    painter.setFont(QFont("Arial", 16, QFont::Bold));
    painter.drawText(200, yPosition, "Liste des Clients");
    yPosition += 40; // Ajouter un espace après le titre

    // Récupérer les noms des clients depuis la base de données
    QSqlQuery query("SELECT nom FROM client"); // Requête SQL
    painter.setFont(QFont("Arial", 12)); // Changer la police pour le texte des noms

    while (query.next()) { // Parcourir les résultats
        QString nom = query.value(0).toString(); // Obtenir le nom du client

        // Vérifier la largeur du texte pour éviter de dépasser la largeur de la page
        QRect textRect = painter.fontMetrics().boundingRect(nom);
        if (textRect.width() > printer.pageLayout().paintRect().width() - 100) {
            // Si le texte dépasse, on réduit la police ou on gère autrement
            painter.setFont(QFont("Arial", 10)); // Réduire la taille de la police
        }

        painter.drawText(50, yPosition, nom);    // Écrire le nom dans le PDF
        yPosition += 20;                        // Passer à la ligne suivante

        // Si l'espace est insuffisant, créer une nouvelle page
        if (yPosition > printer.pageLayout().paintRect().height() - 50) {
            QPageLayout layout = printer.pageLayout();  // Get the current layout
            printer.newPage();  // Create a new page
            yPosition = 30;     // Reset the vertical position
        }
    }
    ...
    QMessageBox::information(this, "Succès", "La liste des clients a été exportée avec succès en PDF !");
}*/
...
/*void MainWindow::sendEmail() {
    ...
    QString recipient = ui->mail->text(); // Email du destinataire
